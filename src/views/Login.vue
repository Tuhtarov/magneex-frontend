      ...
      <v-card class="ma-auto pa-10" :disabled="authLoading">
        ...
        <form class="mx-auto form__login">
          ...
          <div class="d-flex mt-4">
            <v-btn class="mr-4" @click="submit" :loading="authLoading">Войти
              <v-icon dark right>
        ...
        <v-card-text
            v-show="authSuccess === false"
            class="px-0 red--text" color="transparent">
          Ошибка авторизации.
        </v-card-text>
      </v-card>
    </v-row>
...
export default {
  ...
  computed: {
    loginErrors() {
      ...
      !this.$v.login.minLength && errors.push(`Логин должен быть не менее ${MIN_LOGIN} символов.`)
    ...
    passwordErrors() {
      ...
      !this.$v.password.minLength && errors.push(`Пароль должен быть не менее ${MIN_PWD} символов`)
    ...
    async signIn() {
      if (this.login.length > MIN_LOGIN && this.password.length > MIN_PWD) {
        ...
        if (result) {
          ...
          // перезагружаем страницу, что бы обновить стейт
          this.$router.go();
        }
      }
    }
